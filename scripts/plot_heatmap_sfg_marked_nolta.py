...
def plot_heatmap_sfg_marked_nolta(data_df):
  ...
  for i, (organ, ax) in enumerate(zip(organ_list, axes)):
    ...
    df = df.groupby(axis=1,level=0,sort=False).\
      apply(lambda x: (x.T - x.mean(axis=1)).T).abs()
    n = len(df)
    if i==8:
      sns.heatmap(df, cmap='Blues', ax=ax, yticklabels=False, cbar=True,
                  vmin=0, vmax=3, cbar_ax=cax,
                  cbar_kws=dict(label='absolute deviation',
    ...
    ax.plot([i,i,i2,i2,i], [0,n,n,0,0], lw=3, c='C3')
...
if __name__ == '__main__':
  hoge = plot_heatmap_sfg_marked_nolta(data_df)
